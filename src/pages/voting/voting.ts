...
export class VotingPage {
  ...
  loadVoting() {
  	...
  	this.accountData.getVotes(this.accountID).then((votes) => {
	  	if (votes['data'] && votes['data']['votes']) {
		  	this.votes = votes['data']['votes'];
		  	for (let i=0;i < this.votes.length; i++) {
		  		this.votedDelegates.push(this.votes[i]['publicKey']);
		  	}
		  }
	  	this.accountData.getActiveDelegates(101).then((delegates) => {
		  	this.activeDelegates = delegates['data'];
		  	for (let i=0;i < this.activeDelegates.length; i++) {
		  		if (this.votedDelegates.indexOf(this.activeDelegates[i]['account']['publicKey']) != -1) {
  ...
  updateVotes(vote:object) {
  	const index = this.updateVotesArray.indexOf(vote['account']['publicKey']); 
  	if (index == -1) {
  		this.updateVotesArray.push(vote['account']['publicKey']);
  ...
  onScroll(event) { 
    ...
    this.accountData.getStandbyDelegates(50, this.offsetStandby).then((delegates) => {
	  	this.standbyDelegates = delegates['data'];
	  	for (let i=0;i < this.standbyDelegates.length; i++) {
	  		if (this.votedDelegates.indexOf(this.standbyDelegates[i]['account']['publicKey']) != -1) {
  ...
  voteGDT() {
  	...
  	for (let i=0;i < this.gdtMembers.length; i++) {
  		if (this.votedDelegates.indexOf(this.gdtMembers[i]) == -1) {
  			...
  			this.newVotesNames.push(this.gdtMemberNames[i]);
  	...
  	if (remaining < this.newVotes.length-1) {
  		this.showToast(`Not enought free slots to vote all GDT. Have ${remaining} and need ${this.newVotes.length-1}.`);
  		this.newVotes = [''];
  		this.newVotesNames = [''];
  	} else {
	  	
	  	this.newVotes.splice(0, 1);
	  	this.newVotesNames.splice(0, 1);
  ...
  openModal() {
    let myModal = this.modalCtrl.create(VoteModalPage, { newDelegates: this.newVotes, newNames: this.newVotesNames, removeDelegates: this.unVotes, removeNames: this.unVotesNames });
    myModal.present();
  ...
  voteSelected() {
  	for (let i=1;i < this.updateVotesArray.length; i++) {
        ...
        this.unVotes.push(this.updateVotesArray[i]);
        this.unVotesNames.push(this.updateVotesNamesArray[i]);
  			// this.updateVotesArray[i] = `-${this.updateVotesArray[i]}`;
  			// this.updateVotesNamesArray[i] = `-${this.updateVotesNamesArray[i]}`;
  		} else {
        this.newVotes.push(this.updateVotesArray[i]);
        this.newVotesNames.push(this.updateVotesNamesArray[i]);
  			// this.updateVotesArray[i] = `+${this.updateVotesArray[i]}`;
  			// this.updateVotesNamesArray[i] = `+${this.updateVotesNamesArray[i]}`;
  		}
  	} 
  	this.unVotes.splice(0, 1);
  	this.unVotesNames.splice(0, 1);
    this.newVotes.splice(0, 1);
    this.newVotesNames.splice(0, 1);
  	//console.log(this.updateVotesArray);
  	this.openModal();
